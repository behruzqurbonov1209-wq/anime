...
class Database:
    ...
    def add_episode_auto(self, category: str, code: str,
        ...
        """Atomic episode qo'shish — race condition yo'q"""
        with self._conn() as conn:
            cur = conn.execute(
                """INSERT INTO episodes (category, code, episode_num, title, file_id, file_type)
                   SELECT ?, ?, COALESCE(MAX(episode_num), 0) + 1, ?, ?, ?
                   FROM episodes WHERE category=? AND code=?""",
                (category, code.upper(), title, file_id, file_type,
                 category, code.upper())
            )
            # Qo'shilgan episode raqamini qaytarish
            row = conn.execute(
