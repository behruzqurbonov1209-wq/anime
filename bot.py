...
async def admin_poster_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Entry point — no state returned; PTB advances to first state (POSTER_CODE)


async def poster_action_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Action tanlanganda kategoriya so'rash — POSTER_CODE state da ishlaydi"""
    query = update.callback_query
    await query.answer()
    ...
    return POSTER_CODE  # stay in same state — next handler is poster_select_cat


async def poster_select_cat(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Kategoriya tanlangandan keyin kod so'rash — POSTER_CODE state da qoladi"""
    query = update.callback_query
    await query.answer()
    ...
    if action == "add":
        ...
        if not no_poster:
            ...
            return ConversationHandler.END
        hint = ", ".join(i["code"] for i in no_poster[:10])
        await query.edit_message_text(
            f"➕ <b>{cat_name}</b> — poster qo'shish\n\n"
            f"Serial kodini yuboring:\n"
            f"<i>Postersizlar: {hint}</i>",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode="HTML"
        )
    else:
        ...
        if not with_poster:
            ...
            return ConversationHandler.END
        hint = ", ".join(i["code"] for i in with_poster[:10])
        await query.edit_message_text(
            f"🔄 <b>{cat_name}</b> — poster almashtirish\n\n"
            f"Serial kodini yuboring:\n"
            f"<i>Posterlilar: {hint}</i>",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode="HTML"
        )
    # Stay in POSTER_CODE — MessageHandler (poster_get_code) catches the serial code text
    return POSTER_CODE
...
async def poster_get_code(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not cat:
        await update.message.reply_text(
            "❌ Kategoriya tanlanmagan. Qaytadan boshlang.",
...
async def poster_get_img(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data.pop("poster_action", None)
    return ConversationHandler.END


# ══════════════════════════════════════════════════════════
#  KOD HANDLER (FOYDALANUVCHI)
# ══════════════════════════════════════════════════════════
...
def main():
    ...
    poster_conv = ConversationHandler(
        ...
        states={
            ...
            POSTER_IMG: [
                ...
                # Allow re-selecting cat without losing poster_action
                CallbackQueryHandler(poster_select_cat, pattern="^poster_cat_"),
            ],
        },
        fallbacks=[
    ...
    # Conversation handlerlar — group 0
